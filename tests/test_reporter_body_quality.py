"""Reporter roster + body quality + desk fit regressions."""
...
def test_desk_fit_nn_cb_drop():
    lifestyle = {"title": "맛집 여행 팁", "body": "주민 체감 생활 팁과 여행 코스"}
    ok, reason = media_fit_gate("CB", lifestyle)
    assert ok is False
    assert "cb_drop" in reason

    enterprise = {"title": "ESG 공시 의무화", "body": "상장 기업의 공시와 과태료 기준이 마련됐다."}
    ok2, _ = media_fit_gate("CB", enterprise)
    assert ok2 is True

    weak_nn = {"title": "산업 규제 개편", "body": "상장사 공급망 공시 의무만 다룬다."}
    ok3, reason3 = media_fit_gate("NN", weak_nn)
    assert ok3 is False
    assert "nn_drop" in reason3
