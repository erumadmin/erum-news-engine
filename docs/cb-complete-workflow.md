# CB Complete Workflow — Operator Runbook
...
| 재작성 | `cb_packet_writer` + `cb_rewrite_validate` |
| 발행본 | `prepare_cb_publish_body` |
| 품질 루프 | `run_cb_editorial_quality_loop.py` |
...
FIXTURE_URL='https://www.korea.kr/news/policyNewsView.do?newsId=148965573&call_from=rsslink' \
  .venv/bin/python scripts/run_cb_full_pipeline.py --mode review
...
TARGET_URL_IDS='https://www.korea.kr/news/policyNewsView.do?newsId=148965573&call_from=rsslink' \
  .venv/bin/python scripts/run_cb_full_pipeline.py --mode hidden-publish
```
...
|----------|---------|---------|
| `CB_PACKET_PIPELINE` | `1` (via CLI) | 패킷+compliance_brief 재작성 |
| `CB_TARGET_ENGINE` | `1` | compliance_brief 생성 |
| `CB_PUBLISH_V4` | `1` | 본문 무URL + footer |
| `EDITORIAL_FORCE_SITE` | `CB` | 라우팅 강제 |
| `MIN_IMAGE_WIDTH` | `720` | korea.kr og:image |
| `IJ_TARGET_ENGINE` | `0` (CB CLI) | IJ research gate 비활성 |
| `FIXTURE_SOURCE_MD` | — | compare MD에서 원문 주입 |
