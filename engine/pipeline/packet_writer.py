...
- 4번째 <p>는 반드시 「다만」으로 시작하고, 시행 범위·적용 조건·유의점만 쓴다 (비전·홍보·향후 확대 계획 금지).
- 2번째 <p>에 해법 작동(기준·산식·절차·보상 연계)이 보이게 쓴다.
"""
...
- 1번째 <p>: 무엇이·언제·누가 바뀌는지 (뉴스 리드)
- 2번째 <p>: **해법 작동** — 기준·산식·절차·보상 연계 (구조가 보이게)
- 3번째 <p>: 대상·수치·범위·배분·등급 (원문 팩트만)
- 4번째 <p>: 「다만」+ 시행 범위·기간·예외·실질 한계 (비전·홍보 금지)
...
# Keys safe to dump into rewrite user prompt (v4). Drop coalition/NGO briefing noise.
_REWRITE_PACKET_KEEP = (
    "main_claim",
    "key_facts",
    "who_is_affected",
    "conditions",
...
def slim_packet_for_rewrite(packet: dict[str, Any]) -> dict[str, Any]:
    """Shrink packet JSON for rewrite prompts — facts only, no solidarity briefing."""
    out: dict[str, Any] = {}
    for key in _REWRITE_PACKET_KEEP:
        ...
        out[key] = val
    # Cap bulky lists
    if isinstance(out.get("key_facts"), list):
        out["key_facts"] = out["key_facts"][:6]
    if isinstance(out.get("action_items"), list):
        out["action_items"] = out["action_items"][:6]
    return out or {"main_claim": packet.get("main_claim") or "(없음)"}
